        ...
        if is_mouse_button_pressed(MouseButton::Left) {
            ...
            if self.picked_piece.is_some() {
                if let Some(piece_pos) = self.picked_piece_pos {
                    ...
                    if game.board.is_legal_move(piece_pos, pos, game.turn()) {
                        move_made = Some(Move {
                            column_from,
                            row_from,
                            column_to,
